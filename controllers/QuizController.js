...
function postCreateExam(req, res){
    ...
    // const questionsInSub = db.get('subjects').filter('subjectId', data.subjectId).value();
...
function validateCreateExam(data){
    ...
    if(numEasyInSub < data.numMedium ) {
        errors = [...errors, {
            message: 'Số câu trung bình không đủ!'
        }];
    }
    if(numEasyInSub < data.numHard ) {
        errors = [...errors, {
